...
class RxEchoSettings(ttk.Frame):
    def __init__(self, tabctl, root_win=None):
        ...
        n_ports = len(self._popt_handler.get_all_ports().keys())
        ...
        for i in range(len(list(self._popt_handler.get_all_ports().keys()))):
            k = list(self._popt_handler.get_all_ports().keys())[i]
            port = self._popt_handler.get_all_ports()[k]
            var_dict = {}
            ...
            yy = y + 20
            for kk in list(self._popt_handler.get_all_ports().keys()):
                if kk != k:
                    # RX
    ...
    def _check_cmd(self, k, kk):
        ...
        for k in self._popt_handler.port_manager.rx_echo.keys():
                ...
                for kk in list(self.check_vars[k].keys()):
                    ...
                    if self.check_vars[k][kk][0].get() and self.check_vars[kk][k][1].get():
                        ...
                        if not var:
                            ...
                            calls = var.split(' ')
                        self._popt_handler.port_manager.rx_echo[k].rx_ports[kk] = list(calls)
                        self._popt_handler.port_manager.rx_echo[kk].tx_ports[k] = list(calls)
                        ...
                        if kk in list(self._popt_handler.port_manager.rx_echo[k].rx_ports.keys()):
                            del self._popt_handler.port_manager.rx_echo[k].rx_ports[kk]
                        if k in list(self._popt_handler.port_manager.rx_echo[kk].tx_ports.keys()):
                            del self._popt_handler.port_manager.rx_echo[kk].tx_ports[k]
                    ...
                    if self.check_vars[k][kk][1].get() and self.check_vars[kk][k][0].get():
                        ...
                        if not var:
                            ...
                            calls = var.split(' ')
                        self._popt_handler.port_manager.rx_echo[k].tx_ports[kk] = calls
                        self._popt_handler.port_manager.rx_echo[kk].rx_ports[k] = calls
                        ...
                        if kk in list(self._popt_handler.port_manager.rx_echo[k].tx_ports.keys()):
                            del self._popt_handler.port_manager.rx_echo[k].tx_ports[kk]
                        if k in list(self._popt_handler.port_manager.rx_echo[kk].rx_ports.keys()):
                            del self._popt_handler.port_manager.rx_echo[kk].rx_ports[k]
        ...
        for k in self._popt_handler.port_manager.rx_echo.keys():
            if k in self.check_vars.keys():
                for kk in list(self.check_vars[k].keys()):
                    ...
                    # RX
                    try:
                        if k in self._popt_handler.port_manager.rx_echo[kk].tx_ports.keys():
                            call_list_kk_k = self._popt_handler.port_manager.rx_echo[kk].tx_ports[k]
                    ...
                    if kk in self._popt_handler.port_manager.rx_echo[k].rx_ports.keys():
                        call_list_k_kk = self._popt_handler.port_manager.rx_echo[k].rx_ports[kk]
