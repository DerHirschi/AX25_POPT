...
class Yapp(object):
    ...
    def init_tx(self):
        ...
        if self.check_init():
            ...
            self.TX = True
            self.state = 'S'
            self.file_size = len(self.file_rawdata)
            self.e = False
            return True
        return False
    ...
    def dec_RR_RF_RT_AF_AT_CA(self):
        if len(self.rx_pack_buff) >= 2:
            ret = {
                ...
                b'\x04': self.dec_AT,
                b'\x05': self.dec_CA,
            }.get(bytes([self.rx_pack_buff[2]]), False)
            if not ret:
                return False
            return ret()
    ...
    def dec_EF(self):
        """ Send_EOF """
        if len(self.rx_pack_buff) == 2:
            if bytes([self.rx_pack_buff[2]]) == b'\x01':
                self.rx_pac_len = 2
                return 'EF'
        self.rx_pac_len = 0
        return False
