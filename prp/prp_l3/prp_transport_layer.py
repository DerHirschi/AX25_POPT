...
class PRPTransportLayer:
    ...
    def _send_SYN(self, response=False, cmd=True):
        ...
        payload      = bytearray()
        payload      += int(my_no_prio_seq).to_bytes(1, 'big')
        payload      += int(cl_prio_seq).to_bytes(1, 'big')
        payload      += int(cl_no_prio_seq).to_bytes(1, 'big')
    ...
    def _send_FIN(self, response=False, cmd=True):
        ...
        payload      = int(no_prio_seq).to_bytes(1, 'big')
        prp_l3_frame = self._build_l3_pack_w_seq(l3_opt_id=l3_opt_id,
                                                 seq=prio_seq,
                                                 flag1=response,
                                                 flag2=cmd,
